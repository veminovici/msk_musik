...
impl Default for DegreeAlteration {
    /// Returns `DegreeAlteration::Sharp` as the default.
    ///
    /// # Examples
    /// ```
    /// use musik_std::DegreeAlteration;
    ///
    /// assert_eq!(DegreeAlteration::default(), DegreeAlteration::Sharp);
    /// ```
    fn default() -> Self {
        DegreeAlteration::Sharp
    }
...
impl From<u8> for DegreeAlteration {
    ...
    ///
    /// # Panics
    /// Panics if the value is not 2 or 3.
    fn from(value: u8) -> Self {
        match value {
            2 => DegreeAlteration::Flat,
            3 => DegreeAlteration::Sharp,
            _ => panic!(
                "Invalid u8 value for DegreeAlteration: {}. Expected 2 (Flat) or 3 (Sharp)",
                value
            ),
        }
    }
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Invalid u8 value for DegreeAlteration: 0. Expected 2 (Flat) or 3 (Sharp)"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "Invalid u8 value for DegreeAlteration: 1. Expected 2 (Flat) or 3 (Sharp)"
    )]
    fn test_from_u8_invalid_one() {
        let _ = DegreeAlteration::from(1);
    }

    #[test]
    #[should_panic(
        expected = "Invalid u8 value for DegreeAlteration: 4. Expected 2 (Flat) or 3 (Sharp)"
    )]
