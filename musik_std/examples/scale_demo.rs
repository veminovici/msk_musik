...
fn main() {
    ...
    if let Some(d_dorian) = c_major.mode(2) {
        println!("D Dorian (2nd mode): {:?}", 
                 d_dorian.notes().iter().map(|n| n.name()).collect::<Vec<_>>());
    }
    
    if let Some(e_phrygian) = c_major.mode(3) {
        println!("E Phrygian (3rd mode): {:?}", 
                 e_phrygian.notes().iter().map(|n| n.name()).collect::<Vec<_>>());
    }
    ...
    let test_notes_for_analysis = vec![
        Note::G, Note::A, Note::B, Note::C, Note::D, Note::E, Note::FSharp, Note::F
    ];
    
    for note in test_notes_for_analysis {
        if g_major.contains(&note) {
            let degree = g_major.degree_of(&note).unwrap();
            println!("✓ {} is in {} (degree {})", note.name(), g_major.name(), degree);
        } else {
    ...
    println!("{}: {:?}", 
             c_penta_major.name(),
             c_penta_major.notes().iter().map(|n| n.name()).collect::<Vec<_>>());
    
    println!("{}: {:?}", 
             a_penta_minor.name(),
             a_penta_minor.notes().iter().map(|n| n.name()).collect::<Vec<_>>());
    
    // Find common notes
    let common_notes: Vec<_> = c_penta_major.notes()
        .into_iter()
