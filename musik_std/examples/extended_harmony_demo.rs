...
fn main() {
    ...
    println!("  Notes: {} | Semitones: {:?}", major.note_count(), major.semitones());
    ...
    println!("  Notes: {} | Semitones: {:?}", major_extended.note_count(), major_extended.semitones());
    println!("  Bits: {:024b}", major_extended.bits());
    
    // Demonstrate jazz chord extensions
    println!("\n🎷 Jazz Chord Extensions:");
    println!("  9th (major 2nd + octave): {}", if major_extended.contains_semitone(14) { "✓" } else { "✗" });
    println!("  11th (perfect 4th + octave): {}", if major_extended.contains_semitone(17) { "✓" } else { "✗" });
    println!("  13th (major 6th + octave): {}", if major_extended.contains_semitone(21) { "✓" } else { "✗" });
    ...
    println!("  Notes: {} | Semitones: {:?}", dom13_formula.note_count(), dom13_formula.semitones());
    ...
    println!("  Standard (12 semitones): {} notes", chromatic_standard.note_count());
    println!("  Extended (24 semitones): {} notes", chromatic_extended.note_count());
    
    // Demonstrate octave displacement
    println!("\n🔄 Octave Displacement Examples:");
    for semitone in [2, 4, 7, 9] {
        let second_octave = semitone + 12;
        println!("  Semitone {} → Octave higher: {} ({})", 
                 semitone, 
                 second_octave,
                 if major_extended.contains_semitone(second_octave) { "in extended scale" } else { "not in scale" });
    }
